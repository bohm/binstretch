...
// bool debug_position = false; // Erase later.
    ...
    print_if<MEASURE>(
            "(Full, partial) results loaded into knownsum: (%" PRIu64 ", %" PRIu64 ") and %" PRIu64 " losing.\n",
    ...
    do {
        ...
        for (int lowest_sendable_item = 1; lowest_sendable_item <= LOWEST_SENDABLE_LIMIT; lowest_sendable_item++) {
            if (iterated_lc.loadsum() < S * BINS) {
                ...
                if (item < start_item) {
                    ...
                    if (DEBUG) {
                        fprintf(stderr, "Lowsend %d: Even sending %d is a losing move for the conf ",
                                lowest_sendable_item, lowest_sendable_item);
                        print_loadconf_stream(stderr, &iterated_lc, false);
                        fprintf(stderr, "(with total load %d).\n",
                                iterated_lc.loadsum());
                    }
                ...
                if (DEBUG) {
                    fprintf(stderr, "Lowsend %d: Conf is automatically good because it is too large: ",
