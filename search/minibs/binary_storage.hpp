...
public:
    ...
    void read_hash_map_to_ushort(flat_hash_map<uint64_t, unsigned short> &out_fingerprint_map) {
        ...
        int succ_read_values = fread(values, sizeof(unsigned short), map_size, storage_file);
        if (succ_read_values != (int) map_size) {
            ERRORPRINT("Binary storage error: failed to read the values of fingerprint_map.\n");
        }
    ...
    void read_fingerprint_system(flat_hash_map<uint64_t, unsigned short> &out_fingerprint_map,
        ...
        delete[] pointerless;
        read_hash_map_to_ushort(out_fingerprint_map);
    ...
    void write_fingerprint_system(flat_hash_map<uint64_t, unsigned short> &fingerprint_map,
        ...
        write_hash_map_to_ushort(fingerprint_map);
