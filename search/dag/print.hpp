...
void dotprint_victory(victory win, FILE *stream = stderr, bool first = false) {
    ...
    switch (win) {
        case victory::uncertain:
            fprintf(stream, "vict=uncertain");
            break;
        case victory::adv:
            fprintf(stream, "vict=adv");
            break;
        case victory::alg:
            fprintf(stream, "vict=alg");
            break;
        case victory::irrelevant:
            fprintf(stream, "vict=irrel");
            break;
    }
}
...
void dotprint_state(vert_state vs, FILE *stream = stderr, bool first = false) {
    ...
    switch (vs) {
        case vert_state::fresh:
            fprintf(stream, "state=fresh");
            break;
        case vert_state::finished:
            fprintf(stream, "state=finished");
            break;
        case vert_state::expandable:
            fprintf(stream, "state=expandable");
            break;
        case vert_state::expanding:
            fprintf(stream, "state=expanding");
            break;
        case vert_state::fixed:
            fprintf(stream, "state=fixed");
            break;
    }
}
