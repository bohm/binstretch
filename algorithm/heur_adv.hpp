...
int dynprog_and_check_vectors(const binconf &b, const std::vector<loadconf> &v, dynprog_data *dp_data)
{
    return check_loadconf_vectors( dynprog(b, dp_data), v);
}
...
std::pair<bool, loadconf> large_item_heuristic(const binconf& b, dynprog_data *dp_data)
{
    loadconf ret;
    std::vector<loadconf> large_choices = build_lih_choices(b);
    int success = dynprog_and_check_vectors(b, large_choices, dp_data);
...
std::pair<bool, bin_int> five_nine_heuristic(binconf *b, dynprog_data *dp_data)
{
    ...
    bool bins_times_nine_threat = pack_query_compute(*b,9, BINS, dp_data);
	...
	{
	    fourteen_feasible = pack_query_compute(*b,14, fourteen_sequence, dp_data);
	    ...
	    bins_times_nine_threat = pack_query_compute(*b,9, BINS, dp_data);
	}
...
template<minimax MODE> std::pair<victory, adversarial_strategy<MODE>*> adversary_heuristics(binconf *b, measure_attr& meas, dynprog_data *dp_data)
{
    // A much weaker variant of large item heuristic, but takes O(1) time.
    adversarial_strategy<MODE> *str = nullptr;
    if (b->totalload() <= S && b->loads[2] >= R-S)
    {
	if(MODE == mm_state::generating)
	{
	    // Build strategy.
	    heuristic_strategy_list<MODE>* liststrat = new heuristic_strategy_list;
	    std::vector<int> itemlist; 
	    for (int i = 1; i <= BINS-1; i++)
	    {
		itemlist.push_back(S);
	    }
	    liststrat->init(itemlist);
	    liststrat->type = heuristic::large_item;
	    str = liststrat;
	}
	...
	auto [success, heurloadconf] = large_item_heuristic(*b, dp_data);
	if (success)
	{
    ...
    // one heuristic specific for 19/14, currently disabled.
    /* 
    if (S == 14 && R == 19 && FIVE_NINE_ACTIVE && (MODE == mm_state::generating || FIVE_NINE_ACTIVE_EVERYWHERE))
    {

	auto [fnh, fives_to_send] = five_nine_heuristic(b, dp_data);
