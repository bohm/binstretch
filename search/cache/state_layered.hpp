...
public:
    ...
    std::array<std::atomic<conf_el> *, BINS*S+1> hi;
    ...
    void atomic_init_point(int depth, uint64_t point) {
        std::atomic_init(&(hi[depth][point]), conf_el::ZERO);
    }
    ...
    state_layered(uint64_t logbytes, int threads, std::string descriptor = "") {
        ...
        for (int idepth = 0; idepth <= S*BINS; idepth++) {
            ...
            hi[idepth] = new std::atomic<conf_el>[htsize + LINPROBE_LIMIT];
            ...
            for (int w = 0; w < threads; w++) {
                th.emplace_back(&state_layered::parallel_init_segment, this, idepth, start, end, htsize);
                start += segment;
                end += segment;
    ...
    conf_el access(int depth, uint64_t pos) {
        return hi[depth][pos].load(std::memory_order_relaxed);
    }

    void store(int depth, uint64_t pos, const conf_el &e) {
        hi[depth][pos].store(e, std::memory_order_relaxed);
    }
...
inline victory state_layered::lookup_virtual(binconf *bc, int item, unsigned char bin) {
...
void state_layered::insert_binconf(binconf *bc, victory win) {
    conf_el e;
