...
victory computation<MODE, MINIBS_SCALE>::heuristic_visit_alg(int pres_item) {
    ...
    int i = 1;
    if (USING_MINIBINSTRETCHING && HEURISTIC_VISITS_USING_MINIBS) {
    ...
    if (HEURISTIC_VISITS_USING_CACHE) {
        ...
        // Run in case the code above (which fills in alg_uncertain_moves) is not used. Set everything to uncertain.
