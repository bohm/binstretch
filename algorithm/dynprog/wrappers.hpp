...
bin_int dynprog_max_via_vector(const binconf& conf, dynprog_data *dp_data)
{
    std::vector<loadconf> feasible_packings = dynprog(conf, dp_data);
...
bool compute_feasibility(const binconf &h, dynprog_data *dp_data = nullptr)
{
    if (dp_data != nullptr)
    {
	return (DYNPROG_MAX<false>(h,dp_data) != MAX_INFEASIBLE);
    }
...
bool pack_query_compute(binconf &h, const bin_int item, const bin_int multiplicity = 1, dynprog_data *dp_data)
{
    add_item_inplace(h,item, multiplicity);
    ...
    if (!located)
    {
	ret = compute_feasibility(h,dp_data);
	dpc->insert(h,ret);
    } else { // Found in cache.
        ret = feasible;
    }
