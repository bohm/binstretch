...
template<mm_state MODE> victory adversary_strategy_basic<MODE>::heuristics(const binconf *b, thread_attr *tat)
{
...
template<mm_state MODE> void adversary_strategy_basic<MODE>::computation(const binconf *b, thread_attr *tat)
{
...
template<mm_state MODE> void adversary_strategy_basic<MODE>::undo_computation()
{
...
template<mm_state MODE> std::vector<int> adversary_strategy_basic<MODE>::moveset(const binconf *b)
{
    std::vector<int> mset;
...
template<mm_state MODE> void adversary_strategy_basic<MODE>::adv_move(const binconf *b, int item)
{
...
template<mm_state MODE> void adversary_strategy_basic<MODE>::undo_adv_move()
{
    lower_bound.pop();
    itemdepth--;
    if (strat != nullptr)
