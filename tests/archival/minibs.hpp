...
template <int DENOMINATOR> class itemconfig_archival
{
public:
    ...
    itemconfig_archival(const std::array<int, DENOMINATOR>& content)
	{
	    items = content;
	    hashinit();
	}

    itemconfig_archival(const itemconfig_archival& copy)
	{
	    items = copy.items;
    ...
    // We might wish to use itemconfig_archival for the full spectrum of item sizes.
    // If we do, we hit the problem of alignment -- itemconfig_archival only has DENOMINATOR
    ...
    */
    int operator==(const itemconfig_archival& other) const
	{
    ...
    uint64_t virtual_increase(int itemtype) const
	{
	    ...
	    // itemconfig_archival<DENOMINATOR> copy(items);
    ...
    bool inclusion(itemconfig_archival* other)
	{
...
template <int DENOMINATOR> class minidp_archival
{
...
template <int DENOMINATOR> class minibs_archival
{
public:
    ...
    std::vector< itemconfig_archival<DENOMINATOR> > feasible_itemconfs;
    ...
    minidp_archival<DENOMINATOR> mdp;
    ...
    void compute_feasible_itemconfs()
	{
		...
		if (feasible)
		{
		    itemconfig_archival<DENOMINATOR> feasible_itemconf(itemconf);
    ...
    int lb_on_volume(const itemconfig_archival<DENOMINATOR>& ic)
	{
    ...
    bool query_itemconf_winning(const loadconf &lc, const itemconfig_archival<DENOMINATOR>& ic)
	{
    ...
    void init_itemconf_layer(long unsigned int layer_index)
	{
	    ...
	    itemconfig_archival<DENOMINATOR> layer = feasible_itemconfs[layer_index];
...
//			      layer_index, winning_loadconfs, losing_loadconfs);
    ...
    // Backup computations disabled for the archival version.
    
    minibs_archival()
	{
	    fprintf(stderr, "Minibs_Archival<%d>: There will be %d item sizes tracked.\n",DENOM, DENOM - 1);
	    fprintf(stderr, "Minibs_Archival<%d>: There is at most %" PRIu64 " itemconfs, including infeasible ones.\n", DENOM, LAYERS_UB);
