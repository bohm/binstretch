...
// Instead of a while loop, we can use this termination condition whenever ADV relinquishes control.

#define TERMINATE_CONDITION \
    if (calldepth == 0 && stack_state[0] >= 4) { \
...
// if (calldepth == 0) {
//     fprintf(stderr, "ALG returning control to root, stack state %d.\n", stack_state[0]);
// }
...
victory computation<MODE, MINIBS_SCALE>::minimax(adversary_vertex *root_vertex) {
    ...
    //            case 0:
CASE_ZERO:
    assert(stack_state[calldepth] == 0);
...
CASE_TWO:
    ...
    // case 3:
CASE_THREE:
    assert(stack_state[calldepth] == 3);
    ...
    // case 5:
CASE_FIVE:
    assert(stack_state[calldepth] == 5);
    ...
    */
    ...
    // case 6:
CASE_SIX:
    assert(stack_state[calldepth] == 6);
    ...
    //case 7:
CASE_SEVEN:
    assert(stack_state[calldepth] == 7);
    ...
    // case 8:
CASE_EIGHT:
    assert(stack_state[calldepth] == 8);
...
CASE_NINE:
    ...
    assert(stack_state[calldepth] == 9);
...
CASE_TEN:
    ...
    assert(stack_state[calldepth] == 10);
...
CASE_ELEVEN:
    ...
    assert(stack_state[calldepth] == 11);
    ...
    // case 12:
CASE_TWELVE:
    assert(stack_state[calldepth] == 12);
    print_if<MINIMAX_DEBUG>("Minimax depth (%d, %d): State 11 (ALG) entered for "
                            "item %d and binconf ", itemdepth, calldepth, pres_item_to_alg[itemdepth]);
    print_binconf_if<MINIMAX_DEBUG>(bstate);
    ...
    // break;
    // default:
    // fprintf(stderr, "The state machine only works on the prescribed cases. This state %d on calldepth"
    //                 "%d should"
    //                 "never be reached.\n", stack_state[calldepth], calldepth);
    // assert(calldepth >= 0 && stack_state[calldepth] >= 0 && stack_state[calldepth] <= 13);
    // assert(false);
    // break;
    //     }
    // }
